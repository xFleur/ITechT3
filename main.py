...
def twitter_demo():
    ...
    # Get mentions @QuizMeester
    print("mentions")

    mentions = api.GetMentions()
    for mention in mentions:
        print(str(mention.id)+' - ' + mention.text +' - ' )
        print('status_id '+ str(mention.id))
    ...
    useruser = tweet.user.id
    User = api.GetUser(user_id=useruser)
    print('user_name: '+ str(User.screen_name))
    useruserName = User.screen_name
    # reply
    body = str("Reply! HAllo: " + str(useruser) + str(useruserName))
    api.PostUpdate(body, in_reply_to_status_id=status_id_test, auto_populate_reply_metadata=True)

twitter_demo()

        # print(api.GetUser(status_id=1506563385663569924))
        # itech_tweet_id=1506563385663569924
        # api.PostUdate(body, in_reply_to_status_id=itech_tweet_id, auto_populate_reply_metadata=True)

        # result =.api.PostUpdate(body, in_reply_to_status_id=mention.id, auto_populate_reply_metadata=True)
        # api.PostRetweet('@' + mention.user.screen_name +
